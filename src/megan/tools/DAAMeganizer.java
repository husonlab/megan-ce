...
public class DAAMeganizer {
    ...
    public void run(String[] args) throws UsageException, IOException, ClassNotFoundException, CanceledException {
        ...
        ProgramProperties.put(IdParser.PROPERTIES_FIRST_WORD_IS_ACCESSION, options.getOption("-fwa", "firstWordIsAccession", "First word in reference header is accession number", true));
        ...
        for (int i = 0; i < daaFiles.length; i++) {
            ...
            Meganize.apply(new ProgressPercentage(), daaFile, metaDataFile, cNames, minScore, maxExpected, minPercentIdentity, topPercent, minSupportPercent, minSupport, pairedReads, pairedReadsSuffixLength, weightedLCA, weightedLCAPercent);
        }
    }
}
